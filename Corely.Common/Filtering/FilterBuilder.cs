...
public class FilterBuilder<T>
{
    ...
    {
        ValidateMemberAccess(propertySelector.Body);
    ...
    private static void ValidateMemberAccess(Expression expression)
    {
        ...
        if (current is not ParameterExpression)
        {
            throw new ArgumentException(
                "Property selector must be a member access expression (e.g., e => e.Name or e => e.Address.City)."
            );
        }
    }
}
